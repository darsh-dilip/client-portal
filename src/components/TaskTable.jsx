...
// ── Tooltip rendered via a fixed-position div attached to body ─────────────
function AssigneeCell({ uid }) {
  ...
  return (
    <>
      ...
      {open && (
        /* Portal-style: fixed position, never clipped by overflow:hidden parents */
        <div style={{
          position: 'fixed',
          top: pos.top,
          left: pos.left,
          zIndex: 9999,
          /* Fully opaque dark card */
          backgroundColor: '#0f1b2d',
          border: '2px solid rgba(201,168,76,.3)',
          borderRadius: 12,
          padding: '1rem 1.1rem',
          minWidth: 220,
          boxShadow: '0 20px 60px rgba(0,0,0,.95), 0 0 0 1px rgba(0,0,0,.8)',
          pointerEvents: 'auto',
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '.65rem', marginBottom: '.7rem' }}>
            <div style={{ width: 36, height: 36, borderRadius: '50%', flexShrink: 0, background: 'linear-gradient(135deg,#1e3050,#3a7bd5)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '.78rem', fontWeight: 700, color: '#fff' }}>
              {staff?.init || (staff?.name?.[0] || '?').toUpperCase()}
            </div>
            <div>
              <div style={{ fontSize: '.9rem', color: '#f5f0e8', fontWeight: 700 }}>{staff?.name || 'Team Member'}</div>
              <div style={{ fontSize: '.7rem', color: '#8a9bb5', textTransform: 'capitalize', marginTop: '.05rem' }}>{staff?.role || ''}{staff?.dept ? ` · ${staff.dept}` : ''}</div>
            </div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '.45rem', borderTop: '1px solid rgba(255,255,255,.08)', paddingTop: '.7rem' }}>
            {staff?.email && (
              <a href={`mailto:${staff.email}`} style={{ display: 'flex', alignItems: 'center', gap: '.5rem', color: '#b0bed2', fontSize: '.8rem', textDecoration: 'none' }}>
                <Mail size={13} style={{ color: '#c9a84c', flexShrink: 0 }} />{staff.email}
              </a>
            )}
            {staff?.phone && (
              <a href={`tel:${staff.phone}`} style={{ display: 'flex', alignItems: 'center', gap: '.5rem', color: '#b0bed2', fontSize: '.8rem', textDecoration: 'none' }}>
                <Phone size={13} style={{ color: '#c9a84c', flexShrink: 0 }} />{staff.phone}
              </a>
            )}
...
export default function TaskTable({ tasks, title = 'Tasks', externalStatus = null, onClearExternal }) {
  ...
  const activeStatusLabel = { completed:'Filed', overdue:'Overdue', 'in-progress':'In Progress', pending:'Pending' }[statusF] || null;
