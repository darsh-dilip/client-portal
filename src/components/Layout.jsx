...
function statusBadgeConfig(clientStatus) {
  ...
  if (s === 'active') return {
    label: 'Active', color: '#4ecca3',
    bg: 'rgba(78,204,163,.12)', border: 'rgba(78,204,163,.35)', dot: '#4ecca3', disclaimer: false,
  };
  if (s === 'on_hold' || s === 'on hold') return {
    label: 'On Hold', color: '#c9a84c',
    bg: 'rgba(201,168,76,.12)', border: 'rgba(201,168,76,.35)', dot: '#c9a84c', disclaimer: true,
  };
  if (['discontinued','stopped','inactive'].includes(s)) return {
    label: s === 'stopped' ? 'Stopped' : 'Discontinued', color: '#e87070',
    bg: 'rgba(232,112,112,.12)', border: 'rgba(232,112,112,.35)', dot: '#e87070', disclaimer: true,
  };
  return null;
}
...
export default function Layout({ children, user, client }) {
  ...
  return (
    /*
      KEY FIX: outer wrapper is exactly 100vh tall with overflow hidden.
      Sidebar is height:100vh, overflow-y:auto — scrolls independently.
      Main is height:100vh, overflow-y:auto — scrolls independently.
      Neither ever affects the other.
    */
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--navy)' }}>

      {/* ── Sidebar — fixed height, scrolls on its own ─────────── */}
      <aside style={{
        width: 210, flexShrink: 0,
        height: '100vh', overflowY: 'auto',
        background: 'var(--navy-mid)',
        borderRight: '1px solid rgba(201,168,76,.08)',
        display: 'flex', flexDirection: 'column',
      }}>

        {/* BizExpress Logo */}
        <div style={{ padding: '1rem 1rem .85rem', borderBottom: '1px solid rgba(201,168,76,.08)', flexShrink: 0 }}>
          <a href="https://bizexpress.in" target="_blank" rel="noopener noreferrer"
            ...
            <img
              ...
              onError={e => {
                ...
                e.target.parentElement.innerHTML = '<span style="font-weight:700;color:#0f1b2d;font-size:.9rem;">BizExpress</span>';
              }}
            />
          </a>
        </div>
        ...
        <div style={{ padding: '.85rem 1rem', borderBottom: '1px solid rgba(201,168,76,.06)', flexShrink: 0 }}>
        ...
        <div style={{ padding: '.85rem 1rem', borderTop: '1px solid rgba(201,168,76,.08)', flexShrink: 0 }}>
