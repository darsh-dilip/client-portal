...
export default function Layout({ user, client, children }) {
  ...
  useEffect(() => {
    ...
    const event = `VIEW_${route}`;
    logEvent(user, event, location.pathname);
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      ...
      {/* ── Sidebar ──────────────────────────────────────── */}
      <aside style={{
        width: 240, background: 'var(--navy-mid)',
        ...
        <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid rgba(201,168,76,.08)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '.75rem' }}>
            <div style={{
              ...
              justifyContent: 'center', fontSize: '.8rem', fontWeight: 600,
          ...
          {NAV.map(({ to, icon: Icon, label }) => (
            <NavLink key={to} to={to} onClick={() => setOpen(false)}
              style={({ isActive }) => ({
                ...
                padding: '.7rem 1.5rem', fontSize: '.88rem',
